...
/* ─── Provider Card ───────────────────────────────────────────────── */
const ProviderCard = ({ provider, onCall, onAppCall, onGetLocation, showActions }) => {
  const { t } = useLanguage();
        ...
        <View style={{ gap: 10 }}>
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            <TouchableOpacity style={s.callBtn} onPress={onCall}>
              ...
              <Text style={s.callBtnText}>Phone</Text>
            </TouchableOpacity>
            {onGetLocation && (
...
const ServiceRequestDetailScreen = ({ navigation, route }) => {
  ...
  /**
   * In-app voice call via iacax (demo branch).
   *
   * Resolves the callee's mongoId from the request payload — the
   * unified ID system means User._id === Provider._id === jauth userId,
   * so noefix's lookup matches without any extra mapping. iacax mints
   * a LiveKit token and fires the FCM push to noefix in the background;
   * we navigate straight to InCallScreen with mode='outgoing' so the
   * caller sees a "Calling…" state until the callee accepts.
   */
  const handleAppCall = useCallback(async () => {
    const callerIsProvider = isProvider;
    let calleeId, calleeName, calleeType;
    if (callerIsProvider) {
      calleeId = request?.userDetails?._id || request?.userId;
      calleeName = request?.userDetails?.name || request?.userName || t('providerHistory.customer');
      calleeType = 'user';
    } else {
      calleeId = request?.providerDetails?._id || request?.providerId || request?.assignedProviderId;
      calleeName = request?.providerDetails?.name || request?.providerName || t('eventServices.provider');
      calleeType = 'provider';
    }
    if (!calleeId) {
  ...
  }, [isProvider, request, navigation, dialog, t]);
